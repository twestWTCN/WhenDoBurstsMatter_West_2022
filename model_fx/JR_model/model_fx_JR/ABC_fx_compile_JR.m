function [xstore_cond,tvec,wflag,J,Es] = ABC_fx_compile_JR(R,x,uc,pc,m)
% To Do:
...
for condsel = 1:numel(R.condnames)
    ...
    %--------------------------------------------------------------------------
    fx{1} = @ABC_fx_JR_mmc;                                    % motor micro circuit
    ...
    for i = 1:n
        ...
        Q.C  = ones(size(Q.C)).*exp(Q.C)*1000;
        qbank{i} = Q;
    end
    ...
    for tstep = max([R.IntP.bufferExt R.IntP.bufferInt]):R.IntP.nt
        ...
        dW = us{i}(tstep,:).*sqrt(dt).*qbank{i}.C.* m.UInt{i};
